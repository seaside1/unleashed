...
/**
 * The {@link UnleashedCliBlockScript}
 *
 * @author Joseph (Seaside) Hagberg - Initial contribution
 */
public class UnleashedCliBlockScript extends UnleashedAbstractExpectScript {

    private final String aclListName;
    private final String mac;

    public UnleashedCliBlockScript(String username, String password, String host, int port, String aclListName,
            String mac) {
    ...
    public UnleashedExpectStatus execute() {
        try {
            ...
            status = expect.expect(EXPECT_CONFIG_ACL, DEFAULT_EXPECT_TIMEOUT);
            status = expect.enter(UNLEASHED_CMD_ACL_ADD_MAC.concat(mac));
            status = expect.expect(EXPECT_CONFIG_ACL, DEFAULT_EXPECT_TIMEOUT);
